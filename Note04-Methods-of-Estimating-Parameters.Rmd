...
negLogLik = function(A){
  a = A[1]
  b = A[2]
  n*(log(a) + log(b) + (b-1)*sum(log(x)) - a*sum(x^b))
}
## gradient function
grFun = function(A){
  a = A[1]
...
optim(par = c(1,1), fn = negLogLik, gr = grFun, method = "BFGS", 
      control = list(maxit = 20000,fnscale = -1), hessian = TRUE)
```

According to the discussions of local minima and local maxima at the end of section 4.2, the determinant of the Hessian matrix is positive and the top-left corner of the Hessian matrix is less than zero, that is $(\alpha, \beta) = (1.77404763, 0.03477354)$ maximizes the log-likelihood function locally. This means the fitted Weibull distribution has the following density

$$
f(x) = 1.774\times 0.035\times x^{0.035 -1} e^{-1.774 x^{0.035}}.
$$


**Caution**: By default `optim()` performs minimization, we need to set the control statement in the list to a negative value when maximizing the log-likelihood. This is another important point that we need to pay attention to in addition to the convergence code!
