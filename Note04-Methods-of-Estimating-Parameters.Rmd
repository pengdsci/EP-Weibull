...
optim(par = c(-1,1,1), fn = objFun, gr = GrFun, method = "BFGS", hessian = TRUE, control = list(maxit=2000, abstol=0.00000001))
```
With the above **least square estimates (LSE)**, we calculate the gradient vector in the following.


```{r}
GrFun(c( -0.1011996, 6.7443581, -18.2536400))
```

The goodness of the **LSE** is depicted in the following.

```{r}
plot(x,y)
lines(x, -0.101199*x^2+ 6.7443581*x -18.2536400, col="red")
```
...
Before calling `optim()`, we plot the above log-likelihood surface (a function of parameters) in the following.

```{r}
# Data set
dat = c(6.75, 6.83, 7.23, 7.30, 7.50, 7.55, 7.75, 7.89, 7.90,7.94, 8.12, 8.27, 8.32, 8.53, 8.74)
n = length(dat)
x = seq(0.01,100, length = 50)
y = seq(0.01,100, length = 50)
## log-likelihood
zfun <- function(a,b){
  n*log(a) + n*log(b) + (b-1)*sum(log(dat)) - a*sum(dat^b)
}
...
The following estimated gradient vector based on the eestimated parameters is given in the following.

```{r}
grFun(c(1.6091144, 0.22442778))
```

The non-zero vector indicates that the $(\alpha, \beta) = (1.6091144, 0.22442778)$ **is not** the desired MLE!
