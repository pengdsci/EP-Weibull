...
$$
\sqrt{n}(\hat{\theta} - \theta) \stackrel{\text{approx}}{\rightarrow_p} \mathcal{N}\left[\mathbf{0}, \mathbb{I}_0^{-1}(\theta)\right].
$$

Since $\theta$ in the covariance matrix $\mathbb{I}_0^{-1}$ is unknown. When making inferences, we use $\widehat{\mathbb{I}_0(\theta)} = \mathbb{I}_0(\hat{\theta})$.
...
2. $\theta$ is the unknown parameter and $\hat{\theta}$ is the MLE of $\theta$.
...
For convenience, let $(\hat{\theta}_1, \hat{\theta}_2)$ be the MLE of $(\theta_1, \theta_2)$ based on an IID sample $\{x_1, x_2, \cdots, x_n \} \stackrel{\text{i.i.d}}{\sim} f(x, \theta_1, \theta_2)$. The asymptotic sampling distribution of the MLE is assumed to be
...
$$
\text{var}(\hat{\theta}_1) = \frac{\sigma_1^2}{n}, \ \ \text{var}(\hat{\theta}_2) = \frac{\sigma_2^2}{n}, \ \  \text{ and } \ \ \text{cov}(\hat{\theta}_1, \hat{\theta}_2) = \frac{\sigma_{12}}{n}.
$$
...
$$
\text{var}(\hat{\alpha}) = \frac{0.2631785}{45} = 0.005848411  \ \ \text{and} \ \ \text{var}(\hat{\beta}) = \frac{0.4149552}{45} = 0.009221227,
$$
and
...
$$
\left(-\infty, \hat{\theta}_1 + z_{0.95}\frac{\hat{\sigma}_1}{\sqrt{n}}\right) \ \ \text{ and } \ \ \left(\hat{\theta}_1 - z_{0.95}\frac{\hat{\sigma}_1}{\sqrt{n}}, \infty\right)
$$
...
The likelihood ratio test requires restricted and unrestricted MLEs that defined based on the restricted and unrestricted parameter spaces. <font color = “red”>\color{red}This means that the LRT compares only two nested models or distributions.</font>
